...
@CrossOrigin(origins = "http://localhost:3000")//리액트 포트에서 하위 주소 접근 가능하게 해줌 그러나 연결가능은 아님
@RestController//컨트롤러 어노테이션
@RequestMapping("/api")//api라는 하위 주소로 매핑
public class MesController {
    ...
    // GetMapping->테이블 내의 모든 자료 가져옴 findAll이 json 형태로 보여줌
    @GetMapping("/barcode")//   /api/barcode 에서 보여줌
    ...
    //PostMapping->새로운 row 만들 때 쓰는 Post 기능을 가능하게 하는 어노테이션
    @PostMapping("/barcode")
    ...
    // GetMapping ID-> row 별로 자료를 볼 때 필요한 id 별 Get 클래스
...
//PutMapping-> row를 수정할 때 사용되는 어노테이션 set(get)을 통해 구현
    ...
    //DeleteMapping-> row를 삭제하기 위한 어노테이션
